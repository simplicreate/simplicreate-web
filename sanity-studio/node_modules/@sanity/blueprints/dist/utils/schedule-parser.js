 ...
 */
function tryParseTwoTokenTime(token1, token2) {
 ...
 */
function tryParseDayRange(token) {
 ...
 */
function tryParseOrdinal(token) {
 ...
 */
function tryParseInterval(tokens, index) {
 ...
 */
function parseExpressionInternal(expression) {
    const errors = [];
    ...
    if (/\bdaily\b/.test(normalized) || /\bevery\s+day\b/.test(normalized))
        hasDaily = true;
    if (/\bevery\s+minute\b/.test(normalized) || /\bminutely\b/.test(normalized))
        hasMinute = true;
    if (/\bevery\s+hour\b/.test(normalized) || /\bhourly\b/.test(normalized))
        hasHour = true;
    if (/\bfirst\b/.test(normalized) && /\bmonth\b/.test(normalized))
        dayOfMonth = 1;
    // Process tokens
    let i = 0;
...
//# sourceMappingURL=schedule-parser.js.map