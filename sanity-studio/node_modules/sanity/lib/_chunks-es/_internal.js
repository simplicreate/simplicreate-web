...
const debug = debug$3("sanity:backup"), DEFAULT_DOWNLOAD_CONCURRENCY = 10, MAX_DOWNLOAD_CONCURRENCY = 24, helpText$K = `
Options
...
}, DEFAULT_LIST_BACKUP_LIMIT = 30, helpText$I = `
Options
...
`, allowedModes = ["private", "public", "custom"], createDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    try {
      ...
      await client.datasets.create(datasetName, createOptions), output.print("Dataset created successfully");
    } catch (err) {
...
}, EMBEDDINGS_API_VERSION = "v2026-01-01", INITIAL_POLL_INTERVAL_MS = 1e4, MAX_POLL_INTERVAL_MS = 600 * 1e3, POLL_TIMEOUT_MS = 1440 * 60 * 1e3, helpText$z = `
Actions
...
const embeddingsCommand = {
  ...
  action: async (args, context) => {
    const {
      output,
      chalk
    } = context, flags = args.extOptions, [action, dataset] = args.argsWithoutOptions;
    ...
    const {
      ...
      token,
      client
    } = await resolveApiClient(context, dataset, EMBEDDINGS_API_VERSION);
    if (action === "status")
      try {
        ...
        output.print(`Dataset:    ${datasetName}`), output.print(`Embeddings: ${settings.enabled ? "enabled" : "disabled"}`), settings.projection && output.print(`Projection: ${settings.projection}`), output.print(`Status:     ${settings.status}`);
      } catch (error) {
        const {
          message
        } = parseApiErr(error);
        throw new Error(`Failed to get embeddings settings: ${message}`, {
          cause: error
        });
      }
      ...
      try {
        ...
        throw new Error(`Failed to enable embeddings: ${message}`, {
          cause: error
        });
      }
      output.print(chalk.green(`Embeddings enabled for dataset ${datasetName}.`)), projection && output.print(`Projection: ${projection}`), wait ? await waitForReady(output, client, projectId, datasetName, token) : output.print("Indexing documents in the background. Use --wait to wait for completion.");
      ...
      try {
        ...
        throw new Error(`Failed to disable embeddings: ${message}`, {
          cause: error
        });
      }
      output.print(chalk.green(`Disabled embeddings for dataset ${datasetName}.`)), output.print(chalk.yellow("Note: Existing embedding indexes will be removed."));
    }
  }
};
async function waitForReady(output, client, projectId, datasetName, token) {
  const spin = output.spinner("Waiting for embeddings indexing to complete...").start(), deadline = Date.now() + POLL_TIMEOUT_MS;
  let interval = INITIAL_POLL_INTERVAL_MS;
  try {
    for (; Date.now() < deadline; ) {
      await sleep(interval), interval = Math.min(interval * 1.5, MAX_POLL_INTERVAL_MS);
      const settings = await client.request({
        headers: token ? {
          Authorization: `Bearer ${token}`
        } : {},
      ...
      if (settings.status === "ready") {
        spin.succeed(), output.spinner("Embeddings indexing complete.").start().succeed();
        return;
      }
      if (settings.status !== "updating")
        throw spin.fail(), new Error(`Embeddings entered unexpected status: ${settings.status}`);
      spin.text = "Indexing in progress...";
    }
    throw spin.fail(), new Error("Timed out. Check status with: sanity dataset embeddings status");
  } catch (error) {
    throw spin.fail(), error;
  }
}
const noop = () => null, helpText$y = `
Options
